...
@section('title', 'Manuscript Creator')
...
@section('content')
<form action="{{ route('admin.books.store') }}" method="POST" enctype="multipart/form-data" class="flex flex-col h-[calc(100vh-120px)] -m-8">
    @csrf
    <!-- Top Action Bar -->
    <header class="flex items-center justify-between border-b border-slate-200 bg-white px-8 py-4 sticky top-0 z-10">
        <div class="flex items-center gap-4">
            ...
            <div>
                <h2 class="text-xl font-black text-slate-900 leading-tight">Manuscript Creator</h2>
            ...
            <div>
                ...
                <div class="space-y-6">
                    <div class="flex flex-col gap-2">
                        ...
                        <input type="text" name="author" value="{{ old('author') }}" class="w-full bg-slate-50 border-none rounded-xl p-4 text-sm font-bold focus:ring-2 focus:ring-primary transition-all" placeholder="Full name of the author" required>
                    </div>
                    <div class="flex flex-col gap-2">
                    ...
                    <div id="price-container" class="hidden flex flex-col gap-2">
                        <label class="text-[10px] font-black text-slate-400 uppercase tracking-widest">Book Price ($)</label>
                        <input type="number" step="0.01" name="price" value="{{ old('price', '0.00') }}" class="w-full bg-slate-50 border-none rounded-xl p-4 text-sm font-bold focus:ring-2 focus:ring-primary transition-all" placeholder="0.00">
                    </div>
                    <div class="flex flex-col gap-2">
                        <label class="text-[10px] font-black text-slate-400 uppercase tracking-widest">Approx. Pages</label>
                        <input type="number" name="pages" value="{{ old('pages') }}" class="w-full bg-slate-50 border-none rounded-xl p-4 text-sm font-bold focus:ring-2 focus:ring-primary transition-all" placeholder="e.g. 120">
                    </div>
                    <div class="flex flex-col gap-2">
                        ...
                        <div class="relative group cursor-pointer border-2 border-dashed border-slate-200 rounded-2xl aspect-[3/4] flex flex-col items-center justify-center gap-3 hover:border-primary transition-all bg-slate-50/50">
                            ...
                            <div id="image-preview" class="absolute inset-0 rounded-2xl overflow-hidden hidden">
        ...
        <section class="flex-1 bg-slate-50 overflow-hidden flex flex-col">
            ...
            <div class="bg-white border-b border-slate-200 p-3 px-8 flex items-center justify-between">
                ...
                <div class="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em]">
                    Draft Mode • <span id="word-count">0</span> words
                </div>
            </div>
            ...
            <div class="flex-1 overflow-y-auto p-12 custom-scrollbar flex justify-center bg-[#FAF9F6]">
                <div class="max-w-4xl w-full bg-white shadow-2xl rounded-2xl min-h-[1000px] p-20 flex flex-col gap-8">
                    <input type="text" name="title" value="{{ old('title') }}" placeholder="Enter Book Title..." class="text-4xl lg:text-5xl font-black bg-transparent border-none p-0 focus:ring-0 placeholder-slate-200 text-slate-900 leading-tight" required>
                    <textarea name="description" placeholder="Write a brief, compelling description..." class="text-lg text-slate-500 font-medium italic border-none p-0 focus:ring-0 placeholder-slate-200 resize-none min-h-[100px]">{{ old('description') }}</textarea>
                    <div class="h-px bg-slate-100 w-full"></div>
                    <textarea name="content" id="manuscript-editor" placeholder="Start your spiritual manuscript here..." class="flex-1 text-xl leading-relaxed text-slate-700 bg-transparent border-none p-0 focus:ring-0 placeholder-slate-200 resize-none min-h-[600px]" required>{{ old('content') }}</textarea>
                </div>
            </div>
        </section>
    </div>
