...
<div class="max-w-6xl mx-auto space-y-10">
    ...
    <div class="flex items-center justify-between">
        <div class="space-y-1">
            ...
            <h1 class="text-3xl font-black text-slate-900 leading-tight tracking-tight">Edit Book Details <span class="text-slate-300 font-medium mx-3 text-2xl">/</span> <span class="text-slate-400 font-medium">Step 1: Metadata</span></h1>
        </div>
    </div>

    <form action="{{ route('instructor.library.update', $book->id) }}" method="POST" enctype="multipart/form-data" class="space-y-8" id="bookForm">
        @csrf
        @method('PUT')
                    ...
                    <div class="space-y-8">
                        <div class="space-y-3">
                            ...
                            <input type="text" name="title" id="title" value="{{ old('title', $book->title) }}" 
                                   class="w-full h-14 px-6 bg-[#f8f9fb] border-none rounded-2xl focus:ring-2 focus:ring-primary/20 focus:bg-white transition-all font-bold text-slate-900 placeholder:text-slate-400 placeholder:font-medium" required>
                        ...
                        <div class="grid grid-cols-2 gap-8">
                            ...
                            <div class="space-y-3">
                                ...
                                <select name="category" id="category" class="w-full h-14 px-6 bg-[#f8f9fb] border-none rounded-2xl focus:ring-2 focus:ring-primary/20 focus:bg-white transition-all font-bold text-slate-900 appearance-none">
                                    @foreach(['Theology', 'Devotional', 'History', 'Biography', 'Liturgy', 'Other'] as $cat)
                        ...
                        <div class="space-y-3">
                            <div class="flex items-center justify-between">
                                ...
                                <span class="text-[10px] font-bold text-slate-400 uppercase" id="descCount">{{ strlen($book->description) }}/300 characters</span>
                            </div>
                            <textarea name="description" id="description" rows="5" maxlength="300"
                                      ...
                                      placeholder="Briefly describe what this book is about..." onkeyup="countChars(this)">{{ old('description', $book->description) }}</textarea>
                    ...
                    <div class="grid grid-cols-2 gap-12 items-end">
                        ...
                        <div class="bg-[#f8f9fb] p-4 rounded-2xl border border-slate-50 flex items-center justify-between">
                            ...
                            <label class="relative inline-flex items-center cursor-pointer">
                                <input type="checkbox" name="is_free" value="1" class="sr-only peer" {{ $book->is_free ? 'checked' : '' }} onchange="togglePrice(this)">
            ...
            <div class="lg:col-span-4 space-y-8">
                <div class="bg-white p-10 rounded-[2.5rem] border border-[#dcdfe5] shadow-sm space-y-8">
                    ...
                    <div class="relative w-full aspect-[2/3] bg-[#f8f9fb] rounded-3xl border-2 border-dashed border-slate-200 hover:border-primary/50 transition-all group overflow-hidden flex flex-col items-center justify-center cursor-pointer">
                        ...
                        <div class="text-center space-y-4 p-6 {{ $book->cover_image ? 'hidden' : '' }}" id="upload-placeholder">
                        ...
                        <img id="preview" src="{{ $book->cover_image ? asset('assets/books/covers/' . $book->cover_image) : '' }}" class="absolute inset-0 w-full h-full object-cover {{ $book->cover_image ? '' : 'hidden' }}">
                    </div>
                </div>
            </div>
        </div>
